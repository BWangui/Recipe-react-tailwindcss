...
const SingleMeal = () => {
  ...
  const getMealData = async () => {
    ...
    try {
      ...
      // Split and filter instructions by new line characters
      const steps = data.meals[0].strInstructions.split('\n');
      setInstructions(steps.filter(step => step.trim()));
  ...
  // Dispatch action to add meal to cart/favorites
  const handleClick = () => {
    const food = {
      id: meal.idMeal,
            ...
            <div className="flex flex-wrap">
              {Object.keys(meal)
                ...
                .map((key, index) => (
                  <div
                    key={index}
                    className="bg-green-500 text-white w-fit px-3 py-2 m-2 rounded-md font-semibold"
                  >
                    {meal[key]}
                  </div>
                ))}
